...
CWineTest::Run()
{
    ...
    /* The virtual test list is of course faster, so it should be preferred over
       ...
       Enable the journaled one only in case ...
          - we're running under Windivs (as the journal is only useful in conjunction with sysreg2)
    ...
    if(Configuration.IsWindivs() && Configuration.DoCrashRecovery() && Configuration.GetModule().empty())
    {
