 ...
 *              Copyright 2015 J�r�me Gardou <jerome.gardou@reactos.org>
...
CheckSomeDefaultAddresses(VOID)
{
    ...
    if (NT_SUCCESS(Status))
    {
        trace("Below 0x00400000 is available, as on Windivs and Windows S03\n");
        // 0x003F0000, 64k: Free.
        ok_ntstatus(Status, STATUS_SUCCESS);
        Status = NtFreeVirtualMemory(NtCurrentProcess(), &BaseAddress, &Size, MEM_RELEASE);
        ok_ntstatus(Status, STATUS_SUCCESS);
     ...
     * (Windivs and) Windows 95 new default executable image base */
