...
#define _Analysis_assume_section_unlocked_(name)    __AnalysisAssumeUnlockedSection(name)
...
#define __drv_innerMustHoldGlobal(kind,param)       _Pre_ _SA_annotes2(SAL_mustHoldGlobal, #kind, param\t)
#define __drv_innerNeverHoldGlobal(kind,param)      _Pre_ _SA_annotes2(SAL_neverHoldGlobal, #kind, param\t)
#define __drv_innerReleasesGlobal(kind,param)       _Post_ _SA_annotes2(SAL_releaseGlobal,#kind, param\t)
...
#define __drv_Mode_impl(x)                          _Landmark_(x) __inline void __GENSYM(__SAL_dummy_)(void){}
#define __drv_mustHold(kind)                        _When_(!_Arg_comp_(#kind,"Memory"), _Requires_lock_held_(_Curr_)) \
                                                    _When_(_Arg_comp_(#kind,"Memory"), _Kernel_requires_resource_held_(kind))
#define __drv_mustHoldCancelSpinLock                _Requires_lock_held_(_Global_cancel_spin_lock_)
#define __drv_mustHoldCriticalRegion                _Requires_lock_held_(_Global_critical_region_)
#define __drv_mustHoldGlobal(kind,param)            _Requires_lock_held_(param)
#define __drv_mustHoldPriorityRegion                _Requires_lock_held_(_Global_priority_region_)
#define __drv_NDIS                                  __drv_Mode_impl(NDIS)
#define __drv_neverHold(kind)                       _Requires_lock_not_held_(_Curr_)
#define __drv_neverHoldCancelSpinLock               _Requires_lock_not_held_(_Global_cancel_spin_lock_)
#define __drv_neverHoldCriticalRegion               _Requires_lock_not_held_(_Global_critical_region_)
#define __drv_neverHoldGlobal(kind,param)           _Requires_lock_not_held_(param)
#define __drv_neverHoldPriorityRegion               _Requires_lock_not_held_(_Global_priority_region_)
...
#define __drv_preferredFunction(func,why)           _Pre_ _SA_annotes2(SAL_preferredFunction, func, why)
#define __drv_raisesIRQL                            _IRQL_raises_
#define __drv_releasesCancelSpinLock                _Releases_nonreentrant_lock_(_Global_cancel_spin_lock_)
#define __drv_releasesCriticalRegion                _Releases_lock_(_Global_critical_region_)
#define __drv_releasesExclusiveResource(kind)       _Releases_nonreentrant_lock_(_Curr_)
#define __drv_releasesExclusiveResourceGlobal(kind, param) _Releases_nonreentrant_lock_(param)
#define __drv_releasesPriorityRegion                _Releases_lock_(_Global_priority_region_)
...
#define __drv_strictType(typename,mode)             _Pre_ _SA_annotes2(SAL_strictType, typename, mode)
#define __drv_strictTypeMatch(mode)                 _Pre_ _SA_annotes1(SAL_strictTypeMatch,mode)
...
#define __drv_valueIs(arglist)                      _Post_ _SA_annotes1(SAL_return,arglist)
#define __drv_WDM                                   __drv_Mode_impl(WDM)
#define __drv_when(cond, annotes)                   _When_(cond, annotes)
#define __internal_kernel_driver                    _Analysis_mode_(_Analysis_internal_kernel_driver_) \
                                                    _Analysis_mode_(_Analysis_local_leak_checks_)
#define __kernel_code                               _Analysis_mode_(_Analysis_code_type_kernel_code_) \
                                                    _Analysis_mode_(_Analysis_local_leak_checks_)
#define __kernel_driver                             _Analysis_mode_(_Analysis_code_type_kernel_driver_) \
                                                    _Analysis_mode_(_Analysis_local_leak_checks_)
...
#define __user_code                                	_Analysis_mode_(_Analysis_code_type_user_code_) \
                                                    _Analysis_mode_(_Analysis_local_leak_checks_)
#define __user_driver                               _Analysis_mode_(_Analysis_code_type_user_driver_) \
                                                    _Analysis_mode_(_Analysis_local_leak_checks_)
#define ___drv_unit_internal_kernel_driver          _SA_annotes0(SAL_internal_kernel_driver)
#define ___drv_unit_kernel_code                     _SA_annotes0(SAL_kernel) _SA_annotes0(SAL_nodriver)
#define ___drv_unit_kernel_driver                   _SA_annotes0(SAL_kernel) _SA_annotes0(SAL_driver)
#define ___drv_unit_user_code                       _SA_annotes0(SAL_nokernel) _SA_annotes0(SAL_nodriver)
#define ___drv_unit_user_driver                     _SA_annotes0(SAL_nokernel) _SA_annotes0(SAL_driver)
