...
Language=English
Windivs could not be started as configured.  A previous working configuration was used instead.
.
Language=Russian
Windivs could not be started as configured.  A previous working configuration was used instead.
.
Language=French
Windivs n'a pas pu démarrer tel que configuré. Une précédente configuration fonctionnelle a été utilisé à la place.
.
Language=Polish
Nie można uruchomić systemu Windivs zgodnie z aktualną konfiguracją. Zamiast niej użyto poprzedniej działającej konfiguracji.
.
Language=Romanian
Windivs nu a putut fi pornit după configurație. În locul ei, a fost folosită o configurație funcțională anterioară.
.
...
Windivs Management Console (MMC).
.
Language=Romanian
