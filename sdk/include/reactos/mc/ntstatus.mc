...
Language=English
{GUID Substitution}
During the translation of a global identifier (GUID) to a Windivs security ID (SID), no administratively-defined GUID prefix was found.
...
{EXCEPTION}
Cannot Continue
Windivs cannot continue from this exception.
.
...
Language=English
{Bad Image}
%hs is either not designed for Windivs or it contains an error.
...
Language=English
The SAM database on a Windivs Server is significantly out of synchronization with the copy on the Domain Controller. A complete synchronization is required.
.
...
Language=English
The NtCreateFile API failed. This error should never be returned to an application, it is a place holder for the Windivs Lan Manager Redirector to use in its internal error mapping routines.
.
...
Windivs was unable to save all the data for the file %hs. The data has been lost.
...
Language=English
{Windivs Evaluation Notification}
Your Windivs will NEVER expire!.
.
...
The relocation occurred because the DLL %hs occupied an address range reserved for Windivs system DLLs. The vendor supplying the DLL should be contacted for a new DLL.
.
...
Language=English
The Windivs I/O reparse tag passed for the NTFS reparse point is invalid.
.
...
Language=English
The Windivs I/O reparse tag does not match the one present in the NTFS reparse point.
.
...
Your system is low on virtual memory. Windivs is increasing the size of your virtual memory paging file.
...
Language=English
Windivs doesn't have any Small Business Support editions, so this error never appears.
.
...
Language=English
The error specified is not a valid Windivs RPC error code.
.
...
Language=English
Windivs was not able to process the application binding information.
