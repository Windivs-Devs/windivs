...
HRESULT CDefView::FillFileMenu()
{
    ...
    hr = m_pFileMenu->QueryContextMenu(hmenu, 0, DVIDM_CONTEXTMENU_FIRST, DVIDM_CONTEXTMENU_LAST, 0);
    if (FAILED_UNEXPECTEDLY(hr))
        return hr;
...
HRESULT CDefView::OpenSelectedItems()
{
    ...
    hResult = pCM->QueryContextMenu(hMenu, 0, DVIDM_CONTEXTMENU_FIRST, DVIDM_CONTEXTMENU_LAST, CMF_DEFAULTONLY);
    if (FAILED_UNEXPECTEDLY(hResult))
        return hResult;
...
LRESULT CDefView::OnContextMenu(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL &bHandled)
{
    ...
    hResult = m_pCM->QueryContextMenu(m_hContextMenu, 0, CONTEXT_MENU_BASE_ID, DVIDM_CONTEXTMENU_LAST, CMF_NORMAL);
    if (FAILED_UNEXPECTEDLY(hResult))
        return 0;
