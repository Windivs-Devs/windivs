 ...
 * FILE:            lib/ntdll/rtl/hashtable.c
 * PURPOSE:         Hashtable implementation in kernel
 * PROGRAMMERS:     Zafer Balkan <zafer@zaferbalkan.com>
 */
...
/*
Questions:
1. Where to use #if (NTDDI_VERSION >= NTDDI_VISTA) || (REACTOS_KERNEL_TARGET >= NTDDI_VISTA)
...
RtlFlinkToHashEntry(_In_ LIST_ENTRY **FlinkPtr) {
  return _SEH_CONTAINING_RECORD(FlinkPtr, RTL_DYNAMIC_HASH_TABLE_ENTRY, Linkage);
  }

BOOLEAN
NTAPI
RtlExpandHashTable(_In_ RTL_DYNAMIC_HASH_TABLE *HashTable) {
  ...
  if (HT_SECOND_LEVEL_DIR_MIN_SIZE == HashTable->TableSize) {
    SecondLevelDir = HashTable->SecondLevelDir;
    FirstLevelDir = ExAllocatePoolWithTag(
        NonPagedPoolNx, sizeof(LIST_ENTRY *) * HT_FIRST_LEVEL_DIR_SIZE, TAG_HASHTABLE_MEMBER);
  ...
  if (SecondLevelDir == NULL) {
    ...
    SecondLevelDir = ExAllocatePoolWithTag(
        NonPagedPoolNx, RtlComputeSecondLevelDirSize(FirstLevelIndex) * sizeof(LIST_ENTRY),
        TAG_HASHTABLE_MEMBER);
  ...
  if (!IsListEmpty(ChainToBeSplit)) {
    ...
    while (CurEntry->Flink != ChainToBeSplit) {
      ...
      uint32_t BucketIndex =
        ((uint32_t) NextHashEntry->Signature) & ((HashTable->DivisorMask << 1) | 1);

      NT_ASSERT((BucketIndex == (HashTable->Pivot - 1)) ||
...
RtlContractHashTable(_In_ RTL_DYNAMIC_HASH_TABLE *HashTable) {
  ...
  while (!IsListEmpty(ChainToBeMoved)) {
    ...
    RTL_DYNAMIC_HASH_TABLE_ENTRY *HashEntryToBeMoved =
      RtlFlinkToHashEntry(&EntryToBeMoved->Flink);
...
RtlCreateHashTable(
  ...
  if (*HashTable == NULL) {
    Table =
      ExAllocatePoolWithTag(NonPagedPoolNx, sizeof(RTL_DYNAMIC_HASH_TABLE), TAG_HASHTABLE);
    if (Table == NULL) {
      DPRINT1("Allocation of '%s' failed. (%llu bytes)", "RTL_DYNAMIC_HASH_TABLE",
  ...
  if (Table->TableSize <= HT_SECOND_LEVEL_DIR_MIN_SIZE) {
    ...
    SecondLevelDir = ExAllocatePoolWithTag(NonPagedPoolNx, HT_SECOND_LEVEL_DIR_ALLOCATION,
                                           TAG_HASHTABLE_MEMBER);

    if (SecondLevelDir == NULL) {
      DPRINT1("Allocation of '%s' failed. (%llu bytes)", "second level dir (0)",
              HT_SECOND_LEVEL_DIR_ALLOCATION);
   ...
   // fit upper bound.
    ...
    PVOID FirstLevelDir;
    FirstLevelDir = ExAllocatePoolWithTag(NonPagedPoolNx, HT_FIRST_LEVEL_DIR_ALLOCATION,
                                          TAG_HASHTABLE_MEMBER);
    if (FirstLevelDir == NULL) {
      RtlDeleteHashTable(Table);
      return FALSE;
    ...
    for (uint32_t i = 0; i <= FirstLevelIndex; i++) {
      FirstLevelDir[i] = ExAllocatePoolWithTag(NonPagedPoolNx, HT_SECOND_LEVEL_DIR_ALLOCATION,
                                               TAG_HASHTABLE_MEMBER);
      ...
      RtlInitializeSecondLevelDir(FirstLevelDir[i], (i < FirstLevelIndex)
                                                        ? RtlComputeSecondLevelDirSize(i)
                                                        : (SecondLevelIndex + 1));
      }
...
NTAPI
RtlDeleteHashTable(
    _In_ _When_((HashTable->Flags &RTL_HASH_ALLOCATED_HEADER),
      __drv_freesMem(Mem) _Post_invalid_) RTL_DYNAMIC_HASH_TABLE *HashTable) {
  ...
  if (HashTable->TableSize <= HT_SECOND_LEVEL_DIR_MIN_SIZE) {
   ...
   // never be used.
    if (HashTable->FirstLevelDir != NULL) {
      ...
      RtlComputeDirIndices((HashTable->TableSize - 1), &largestFirstLevelIndex,
                           &largestSecondLevelIndex);
#endif
...
BOOLEAN
NTAPI
RtlInsertEntryHashTable(_In_ RTL_DYNAMIC_HASH_TABLE *HashTable,
  ...
  if (HashTable->NumEntries >
      RTL_DYNAMIC_HASH_TABLE_MAX_CHAIN_LENGTH * HashTable->NonEmptyBuckets) {
...
BOOLEAN
NTAPI
RtlRemoveEntryHashTable(_In_ RTL_DYNAMIC_HASH_TABLE *HashTable,
  ...
  if (EmptyBuckets >
      (RTL_DYNAMIC_HASH_TABLE_MAX_EMPTY_BUCKET_PERCENTAGE * HashTable->TableSize / 100)) {
    uint32_t RestructAttempts = RTL_DYNAMIC_HASH_TABLE_MAX_RESIZE_ATTEMPTS;
    do {
    ...
    while ((RestructAttempts > 0) &&
          (EmptyBuckets > (RTL_DYNAMIC_HASH_TABLE_MAX_EMPTY_BUCKET_PERCENTAGE *
...
PRTL_DYNAMIC_HASH_TABLE_ENTRY
NTAPI
RtlLookupEntryHashTable(_In_ RTL_DYNAMIC_HASH_TABLE *HashTable,
                        _In_ ULONG_PTR Signature,
...
PRTL_DYNAMIC_HASH_TABLE_ENTRY
NTAPI
RtlGetNextEntryHashTable(_In_ RTL_DYNAMIC_HASH_TABLE *HashTable,
                         _In_ RTL_DYNAMIC_HASH_TABLE_CONTEXT *Context) {
...
BOOLEAN
NTAPI
RtlInitEnumerationHashTable(_In_ RTL_DYNAMIC_HASH_TABLE *HashTable,
...
PRTL_DYNAMIC_HASH_TABLE_ENTRY
NTAPI
RtlEnumerateEntryHashTable(_In_ RTL_DYNAMIC_HASH_TABLE *HashTable,
...
VOID
NTAPI
RtlEndEnumerationHashTable(_In_ RTL_DYNAMIC_HASH_TABLE *HashTable,
                                      _Inout_ RTL_DYNAMIC_HASH_TABLE_ENUMERATOR *Enumerator) {
...
BOOLEAN
NTAPI
RtlInitWeakEnumerationHashTable(_In_ RTL_DYNAMIC_HASH_TABLE *HashTable,
                                _Out_ RTL_DYNAMIC_HASH_TABLE_ENUMERATOR *Enumerator) {
...
PRTL_DYNAMIC_HASH_TABLE_ENTRY
NTAPI
RtlWeaklyEnumerateEntryHashTable(_In_ RTL_DYNAMIC_HASH_TABLE *HashTable,
                                 _Inout_ RTL_DYNAMIC_HASH_TABLE_ENUMERATOR *Enumerator) {
...
VOID
NTAPI
RtlEndWeakEnumerationHashTable(_In_ RTL_DYNAMIC_HASH_TABLE *HashTable,
                                          _Inout_ RTL_DYNAMIC_HASH_TABLE_ENUMERATOR *Enumerator) {
