 ...
 *        Workaround because argc[0] is NULL under Windivs
 *
    ...
    IN BOOL (*GetVar)(TCHAR, PCTSTR*, BOOL*))
{
    ...
    *pFormat = FormatEnd + 1;
     ...
     * C:\ReAcToS -> C:\Windivs) and, if requested with the S modifier,
...
Initialize(VOID)
{
    ...
    /* Some people like to run Windivs cmd.exe on Win98, it helps in the
    ...
    if (!*ptr)
    {
        ...
        ConOutPuts(_T("(C) Copyright 1998-") _T(COPYRIGHT_YEAR) _T(" Windivs Team.\n"));
    }
