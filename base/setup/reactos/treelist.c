...
typedef struct {
	...
	COLORREF	uColors[MAX_COLORS];						// 0=Hintergrundfarbe  1=Abwechselnte Farbe  2=Farbe f�r Trennlinien  3=Textfarbe
	...
	unsigned	uTreeItemsMax;								// Gr��e der Liste mit den vorhanden Eintr�ge (alociert um 1 gr��er)
	...
	unsigned	uToolTipShow;								// Ist die Zeitverz�gerung in 500 ms Schritten f�r das Tooltip
	...
	unsigned	uTrippleN;									// Bereite des "..." Strings f�r den normalen Fonts
...
static int UpdateColumns(TreeListData *pData) {
	...
	for(uCol = 0; uCol < pData->uColumnCount;) {					// Suche die erste ge�nderte Spalte
...
static int TreeListSetOrderArray(TreeListData *pData, unsigned uItems, unsigned *pArray) {
	...
	if(pData->uStyleEx & TVS_EX_HEADERCHGNOTIFY) {				// Alle Spalten haben sich ver�ndert
		UNLOCK(pData);
...
static int TreeListToggleItem(TreeListData *pData, unsigned uItem, unsigned uAddFlags) {
	...
	if(pData->uSelectedItem && bDo) {							// Ist der ausgew�hlten Eintrag sichtbar ?
...
static int TreeListDeleteItem(TreeListData *pData, unsigned uItem, int iMode) {
	...
	if(uItem == pData->uSelectedItem) {							// Einen ausgew�hlten Eintrag l�schen
		...
		UNLOCK(pData);
		SendNotify(pData, &sNotify.hdr);						// Bekant geben das der Eintrag nicht mehr ausgew�hlt ist
		LOCK(pData);
	...
	for(uPos = 1; uPos < pData->uColumnCount; uPos++) {				// Alle Extraeintr�ge l�schen
...
static int TreeListSelectItem(TreeListData *pData, unsigned uItem, unsigned uSubItem, int iMode) {
	...
	if(pData->uStyleEx & TVS_EX_MULTISELECT) {					// Ist die Mehrfachauswahl m�glich
	...
	if(SendNotify(pData, &sNotify.hdr)) {						// Abfragen ob der Eintrag gew�hlt werden darf
		LOCK(pData);
		return 0;
	}
	...
	UNLOCK(pData);
	uRet = U(SendNotify(pData, &sNotify.hdr));					// Anfragen ob die Zweige umgeklappt werden d�rfen
	LOCK(pData);
...
static unsigned TreeListInsertItem(TreeListData *pData, TV_INSERTSTRUCT *pInsert) {
	...
	if(pData->uTreeItemsCount + 1 > pData->uTreeItemsMax) {		// Gr��e der Liste erh�hen
	...
	switch(uAfter) {
		...
		case U(TVI_SORT):										// RCpetisch einf�gen
			uItem = pFirst[0];
			if(!uItem) {										// Gibt es keine Kindeintr�ge
				pFirst[0] = uPos;
				pLast [0] = uPos;
...
DoInsert:
			...
			if(uAfter > pData->uTreeItemsMax) {
				if((uAfter & 0xFFF00000) == 0xFFE00000) {			// In einer genauen Reihe nach Patent einf�gen
					uAfter &= 0xFFFFF;
...
//*
//*****************************************************************************
//	L�scht eine Spalte aus dem Header
//	pData		: Zeiger auf die Fensterdaten
//	uCol		: Ist die Nummer der Spalte die gel�scht werden soll
//	Ergibt 1 wenn die Spalte gel�scht wurde
static int TreeListDeleteColumn(TreeListData *pData, unsigned uCol) {
	...
	for(uPos = 0; uPos < pData->uColumnCount; uPos++) {				// Z�hlern der variablen Spalten
		if(uPos == uCol)
			continue;
	...
	if(pData->uColumnCount > 0) {								// Liste mit Extraeintr�gen l�schen
		iNum = uCol - 1;
		if(iNum < 0)
			iNum = 0;
		...
		if(pList) {
			for(uItem = 0; uItem <= pData->uTreeItemsMax; uItem++) { // Alle Eintr�ge aus der Liste l�schen
				pExtra = pList[uItem];
				if(!pExtra)
					continue;
	...
	if(!pData->uColumnCount) {								// Den Header l�schen
	...
	iXoff = UpdateColumns(pData);							// Hat sich die Spaltenbreiten ver�ndert
	if(iXoff < 0x10000) {
		sRect.left  = iXoff;
		sRect.left -= pData->uScrollX;
		sRect.top   = pData->uStartPixel;
...
//	pData		: Zeiger auf die Fensterdaten
//	uCol		: Ist die Nummer der Spalte die eingef�gt wird
//	pInsert		: Zeiger auf die ein zu f�genden Daten
...
static int TreeListInsertColumn(TreeListData *pData, unsigned uCol, TV_COLUMN *pColumn) {
	...
	if(pData->uColumnCount >= MAX_COLUMNS) {					// Pr�fe die Anzahl der Spalten
		return -1;
	}
	...
	if(pColumn->mask & TVCF_TEXT) {								// Auch einen Text �bergeben
		sItem.mask    |=  HDI_TEXT;
		sItem.pszText  =  pColumn->pszText;
	}

	if(pColumn->mask & TVCF_MIN) {								// Auch einen Min-Wert �bergeben
		iMin		   =  pColumn->iOrder;
		bMinEx		   =  1;
	...
	if(pColumn->mask & TVCF_WIDTH) {							// Fixe Breite f�r die Spalte
		iWeight		   =  0;
		sItem.mask    |=  HDI_WIDTH;
	...
	if(pData->uColumnCount > 0) {								// Liste mit Extraeintr�gen erzeugen
	...
	for(uIndex = pData->uColumnCount;;) {						// N�chste sichtbare Spalten aktualisieren
		bByte						 = pData->aColumn[uIndex].bIndex;
		pData->aColumn[uIndex].bNext = pData->aColumnPos[bByte + 1];
		if(uIndex == 0)
			break;
	...
	if(pData->uInsertMark) {									// Fehlende Infomarken einf�gen
		TV_ITEM		sSet;
		ExtraItem  *pExtra;
	...
	if(pData->uStyleEx & TVS_EX_HEADERCHGNOTIFY) {				// Ge�nderte Spalten melden
		sNotify.hdr.code			= TVN_COLUMNCHANGED;
		sNotify.uColumn				= uCol;
		sNotify.uIndex				= pData->aColumn[uCol].bIndex;
		sNotify.uPosX				= pData->aColumnXpos[uCol];
...
//*
//*****************************************************************************
//	Berechnet die Breite der sichtbaren Eintr�ge einer Spalte
//	pData		: Zeiger auf die Fensterdaten
//	uSub		: Ist die Spalte
...
//*
//*****************************************************************************
//	Pr�ft wo eine Koordinate im Fenster ist
//	pData		: Zeiger auf die Fensterdaten
//	pInfo		: Zeiger auf die ein zu f�genden Daten
...
static unsigned TreeListHitTest(TreeListData *pData, TV_HITTESTINFO *pInfo) {
	...
	if(iXpos < pData->iIndent) {								// Auf einger�cktem Bereich
...
//*
//*****************************************************************************
//	Setzt das den Einf�geeintrag
//	pData	: Zeiger auf die Fensterdaten
//	uItem	: Ist die Nummer des Eintrages bei dem eingef�gt werden soll
//	iMode	: 0=davor einf�gen  1=nachher einf�gen
//	Ergibt 1 wenn der Eintrag eingef�gt wurde
...
static COLORREF TreeListSetItemColor(TreeListData *pData, unsigned uItem, unsigned uSub, COLORREF uColor, int iMode) {
	...
	if(uSub >= 255) {											// Die Ganze Zeile �ndern
	...
	if(uSub) {												// Extra-Eintrag ver�ndern
		if(uSub >= pData->uColumnCount)
			return TV_NOCOLOR;
...
static int TreeListSetTrackItem(TreeListData *pData, unsigned uItem, unsigned uSub) {
	...
	if(pData->uTrackedItem) {									// Den alten Eintrag zur�cksetzen
...
static unsigned TreeListFindItem(TreeListData *pData, unsigned uItem, TVFIND *pFind) {
	...
	if(pFind->uFlags & TVIF_CHILD) {						// In den Kindern suchen
		...
		if(uItem > pData->uTreeItemsMax) {					// Ist der Eintrag g�ltig
			return 0;
		}

		if(pFind->uFlags & TVIF_NEXT) {						// Beim n�chsten Eintrag weitersuchen
			pEntry = pData->pTreeItems[uItem];
			if(!pEntry)
				return 0;
...
//*
//*****************************************************************************
//	Sucht rekursiv ausgew�hlte Eintr�ge
//	pData	: Zeiger auf die Fensterdaten
...
//*
//*****************************************************************************
//	Sucht rekursiv nicht ausgew�hlte Eintr�ge
//	pData	: Zeiger auf die Fensterdaten
//	uItem	: Ist die Nummer des Eintrages nach dem mit der Suche begonnen werden soll
//	uStop	: Bestimmt bei welchem Eintrag abgebrochen werden soll werden soll
//	Ergibt die Nummer des Eintrages oder 0 wenn keiner gefunden wurde
...
//	pData	: Zeiger auf die Fensterdaten
//	uItem	: Ist der Eintrag der ge�ndert werden soll
//	pInfo	:
static void TreeListChangeCheckbox(TreeListData *pData, UINT uItem, int iPosX, int iPosY) {
	...
	if(pData->uStyleEx & TVS_EX_SINGLECHECKBOX) {			// Einzelauswahl
		...
		if(pData->uSingleSel == uItem) {
			...
			if(pData->uSingleSel && pTemp) {				// Anderer Eintrag gew�hlt
...
//*
//*****************************************************************************
//	Soll ein Mausklick ausgef�hrt werden. ACHTUNG UNLOCK wird ausgef�hrt
//	pData	: Zeiger auf die Fensterdaten
//	uMsg	: Message des Mausklicks
//	wParam	: WPARAM des Mausklicks
//	lParam	: LPARAM des Mausklicks
static void TreeListMouseClick(TreeListData *pData, UINT uMsg, WPARAM wParam, LPARAM lParam) {
	...
	if(uItem) {											// Wurde auf einen Eintrag getr�ckt
		pEntry = pData->pTreeItems[uItem];
		...
		if(sInfo.flags & TVHT_ONITEMBUTTON) {				// Eintrag aufklappen
			...
			if(sInfo.flags & TVHT_ONITEMSTATEICON) {		// Checkbox umschalten
				...
				if(sInfo.flags & uMaskItem) {					// Eintrag ausw�hlen
					...
					if(pData->uStyleEx & TVS_EX_MULTISELECT) {		// Mehrfachauswahl erlaubt
						if(uMsg == WM_RBUTTONDOWN)					// Keine Abwahl wenn mehrer Eintr�ge aus gew�hlt
						...
						if(uMsg == WM_LBUTTONDOWN)					// Spezialsteuerung f�r Multiselect bei Darg
						...
						if(wParam & MK_SHIFT) {						// Bis zum angeklicken Ausw�hlen
							if(uMsg != WM_LBUTTONDOWN)
							...
							for(;; uPos += iAdd) {					// Eintr�ge w�hlen
								...
								if(pTemp && !pTemp->uShowPos) {
									if(uSel) {
										...
										for(uNum = uTemp;;) {		// Kinder abw�hlen
						...
						if(wParam & MK_CONTROL) {					// Einzelen Eintrag umschalten
							...
							if(pEntry && (pEntry->uState & TVIS_SELECTED)) {
								...
								if(TreeListXorSelectItem(pData, uItem, TVC_BYMOUSE)) {
									// Auch unsichtbare Kinder abw�hlen
									pTemp =   pData->pTreeItems[pEntry->uFirstChild];
									if(pTemp && !pTemp->uShowPos)
										for(uTemp = uItem;;) {
					...
					if(wParam & MK_CONTROL) {						// Ist die Ctrl-Taste gedr�ckt
						iMode = TVC_BYMOUSE;
					} else {
					...
					if(sInfo.flags & uMaskSub) {					// Extra-Eintrag ausw�hlen
						if(pData->uStyleEx & TVS_EX_SUBSELECT)
							uSub = TVHT_SUBTOCOL(sInfo.flags);
						else
							uSub = 0;
...
//*
//*****************************************************************************
//	Eingabe von Buchstaben �ber die WM_CHAR Nachricht
//	pData	: Zeiger auf die Fensterdaten
	...
	if(nChar >= ' ') {
		...
		if(!(pData->uStyleEx & TVS_EX_NOCHARSELCET))
			for(;;) {										// Suche Anfangsbuchstaben
				...
				for(i = iNum + 1; i != iNum; i++) {				// Suche �bereinstimmung
...
//*
//*****************************************************************************
//	Soll ein Tastentruck ausgef�hrt werden. ACHTUNG UNLOCK wird ausgef�hrt
//	pData	: Zeiger auf die Fensterdaten
//	wParam	: WPARAM des Mausklicks
//	lParam	: LPARAM des Mausklicks
...
static void TreeListKeyDown(TreeListData *pData, WPARAM wParam, LPARAM lParam) {
	...
	if(wParam == VK_RETURN) {								// Wurde Enter gedr�ckt
		UNLOCK(pData);
	...
	if(iDel && iScr)										// Ende oder Anfang w�hlen
	...
	if(iDel && iScr) {									// Das Fenster scrollen
		...
		switch(wParam) {
			...
			case VK_SPACE:
				if(pEntry == NULL)
					break;				// Expantieren und schlie�en
				if(pEntry->bFlags & TVIX_HASBUTTON) {
	...
	} else {													// Einen anderen Eintrag ausw�hlen
		iSub  = pData->uSelectedSub;
		...
		if(pData->uItemPosCount > 0)
			if(iLine != iOldLine || iCol != iOldCol) {			// Wurde ein anderer Eintrag ausgew�hlt
				if(pData->uSelectedCount > 1) {
					TreeListRemoveFocus(pData);
				} else {
					pData->uSelectedBase = uTemp;
				...
				if(pData->cReSelect && pData->uSelectedBase) {	// Shift-Select neu ausw�hlen
					uTemp = pData->uSelectedBase;
					pTemp = pData->pTreeItems[uTemp];
					uStop = uItem;
				...
				if((pData->uStyleEx & TVS_EX_MULTISELECT) && !iDel) {
					...
					for(; iPos >= iMax; iPos--) {					// �bersprungene Eintr�ge mit aus/abw�hlen
						...
						pTemp = pData->pTreeItems[uItem];	// Auch unsichtbare Kinder abw�hlen
						if(pTemp) {
...
//*
//*****************************************************************************
//	Sortiert die Kindereintr�ge eines Eintrages mitteles einer Funktion
//	pData		: Zeiger auf die Fensterdaten
...
static int TreeListSortItemsEx(TreeListData *pData, TV_SORTEX *pSortData, int iMode) {
	...
	if(iMode) {											// Sortiere die Untereintr�ge
		pNext				= pList[uFirst];
		sSort.hParent		= (HTREEITEM)(ULONG_PTR)uFirst;
		sSort.lParam		= pSortData->lParam;
		sSort.lpfnCompare	= pSortData->lpfnCompare;
...
//******************** Eintr�ge neu einsortirenen *****************************
	...
	for(uNum = 0; uNum < uPos;) {								// Kinder neu einh�ngen
		pEntry = pList[uItem];
...
//*
//*****************************************************************************
//	Sortiert die Kindereintr�ge eines Eintrages via Text
//	pData		: Zeiger auf die Fensterdaten
//	pSortData	: Ist ein Zeiger auf die Sortiertaten
//	iMode		: 1=Rekursiv sortieren
//	Ergibt 1 wenn ok sonst 0
...
static int TreeListEndLabelEdit(TreeListData *pData, int iMode) {
	...
	if(uSub) {											// Wurde der Text in einer Sub-Spalte ge�ndert
		pExtra = pData->pExtraItems[uSub - 1][uItem];
		if(!pExtra) {
	...
	if(cCb) {												// Callback aufrufen
		...
		if(pData->aColumn[uSub].bFlags & TVAE_NEXTLINE &&			// In die n�chste Zeile springen
		        iMode == 2 && iAuto != TVAX_NONE && iAuto != TVAX_STEP) {
			if(!pData->cColumnStart) {
				if(uSub) {									// Hat sich der Text ver�ndert ?
					pExtra = pData->pExtraItems[uSub - 1][uItem];
...
static HWND TreeListEditLabel(TreeListData *pData, unsigned uItem, unsigned uSub) {
	...
	if(pData->hEdit) {									// Editfenster l�schen
...
static int TreeListStartNotifyEdit(TreeListData *pData, unsigned uItem, unsigned uSub, WPARAM wParam, LPARAM lParam) {
	...
	if(wParam >= ' ' && wParam <= 0xFFFF) {					// Shift und Cltr pr�fen
	...
	if(lRet & TVIR_EDITCOMBOBOX) {							// Die Combobox f�llen
		uMax = sNotify.uMaxEntries;
		iSel = -1;

		if(sNotify.pTextList) {							// Texte �ber Listenfeld
			...
			for(uCnt = 0; uCnt < uMax; uCnt++) {
				for(uLen = 0; pText[uLen]; uLen++) {			// Subtextl�nge holen
					if(pText[uLen] == cChar)
						break;
				}
			...
			if(lRet & TVIR_EDITCOMBODEL) {					// Den Puffer l�schen
				delete((TCHAR*)sNotify.pTextEntries);
			}
		}

		if(iSel >= 0) {										// Listeneintrag ausw�hlen
			SendMessage(hWnd, CB_SETCURSEL, iSel, 0);
		}

		if(sNotify.uHeight) {								// H�he der Dropdown-Liste einstellen
...
//*
//*****************************************************************************
//	Startet die Autoeditierung f�r eine Spalte
//	pData		: Zeiger auf die Fensterdaten
...
//******************** Weiterschalten mit Return ******************************
	if(uMode >= TVAX_STEP) {
		...
		if((uMode & 1) && wParam != VK_ICONCLK) {					// Ein Edit-Feld �ffnen
			uMode = TVAX_EDIT;
			goto EditField;
		}
		...
		if(uBits & TVAE_PTRLIST) {								// Zeigerliste  char *pTexte[]={"1","2",NULL};
			...
			for(uPos = 0;; uPos++) {
				...
				for(uLen = 0; pText[uLen]; uLen++) {				// Subtextl�nge bestimmen
					if(pText[uLen] == cChar)
						break;
				}
			...
			for(uLen = 0; pText[uLen] && uLen < 256; uLen++) {		// Ausgew�hlten Text kopiren
				if(pText[uLen] == cChar)
					break;
...
EditField:
	...
	// Ersten Buchstaben ans Fenster �bergeben
...
//*
//*****************************************************************************
//	Ist die Fensterfunktion f�r das TreeList Fenster
static LRESULT CALLBACK TreeListProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
	...
	switch(uMsg) {
		...
		case WM_DESTROY:											// Das Fenster zerst�ren
		...
		case WM_NCDESTROY:										// Das Fenster zerst�ren
		...
		case WM_SIZE:												// Die Fenstergr��e wurde ver�ndert
			...
			if(uVal && uVal != pData->uSizeX) {
				...
				if(pData->uColumnCountVar) {					// Spalten mit variabler Breite nach f�hren
					RECT	sRect;
					int		iDelta;
					int		iNum;
				...
				if(uVal > uOld) {								// Hat sich die Breite vergr��ert
					RECT	sRect;
			...
			if(uVal && uVal != pData->uSizeY) {
				if(uVal > pData->uSizeY) {						// Hat sich die H�he vergr��ert
					RECT	sRect;
				...
				pData->uMaxEnties   /= pData->iRowHeight;
				pData->uPageEnties  /= pData->iRowHeight;
				// Wenn H�he vergr��ert dann Scroll-Position pr�fen
		...
		case WM_LBUTTONDOWN:										// Mausklick
			...
			if(pData->cButtonFlag && pData->uToolTipItem) {	// Doppelklick simulieren �ber ToolTip
				pData->cButtonFlag = 0;
		...
		case WM_SYSCOLORCHANGE:										// Wurden die Systemfarben ver�ndert
		...
		case WM_STYLECHANGED:										// Hat sich der Fenstersytle ge�ndert
		...
		case WM_TIMER:												// Timer Funktion
			...
			if(wParam == ID_TOOLTIPCHECK) {
				pData	= GetHandle(hWnd);
				if(pData->uToolTipItem) {
					if(pData->uToolTipShow) {					// Verz�gertes einblenden
						pData->uToolTipShow--;
		...
		case WM_NOTIFY:												// Notify Nachrichnten
			...
			if(wParam == 1) {									// Nachricht vom Header
				...
				iCode = pHdr->hdr.code;
				// Hat sich die Spaltenbreite ver�ndert
				...
				if(iCode == HDN_BEGINTRACK) {					// User will die Spaltenbreite �ndern
					pData	= GetHandle(hWnd);
					...
					if(pData->uStyleEx & TVS_EX_NOCOLUMNRESIZE) {	// Darf der User die Spaltenbreite �ndern
						return 1;
					}
					...
					if(pData->uStyleEx & TVS_EX_FIXEDCOLSIZE) {	// Fixe gesammte Spaltenbreite
						...
						for(iNext++;; iNext++) {				// Suche n�chste ver�nerbare Spalte
					...
					if(U(pHdr->iItem) < pData->uColumnCount)		// Darf die Spalte ver�ndert werden
						if(pData->aColumn[pHdr->iItem].sFixed) {
							...
							GetCursorPos(&sPoint);					// Die n�chste ver�nderbare Spalte greifen
				...
				if(iCode == HDN_DIVIDERDBLCLICK) {				// Doppelcklick auf Spaltentrenner
					...
					if(iSize) {								// Spalte auf maximale Textbreite
						...
						if(pData->aColumn[pHdr->iItem].bMinEx)	// Minimale Breite pr�fen
				...
				if(iCode == HDN_TRACK || iCode == HDN_ENDTRACK) {	// Wurde die Spaltenbreite ver�ndert
					pData	= GetHandle(hWnd);
					...
					if(pData->aColumn[iCol].bMinEx)				// Minimale Breite pr�fen
					...
					if(pData->uStyleEx & TVS_EX_FIXEDCOLSIZE) {	// Fixe gesammte Spaltenbreite
						...
						for(iNext++;; iNext++) {				// �berspringe fixierte Spalten
							if(U(iNext) >= pData->uColumnCount) {
								UNLOCK(pData);
								return 0;
							}
						...
						if(iDelta < 0) {						// N�chste Spalte wird zu klein
							if(sTemp.cxy < 0) {
								sTemp.cxy  =  0;
							...
							if(pData->aColumn[iSub].bMinEx)		// Minimale Breite pr�fen
		...
		case TVM_GETSETOPTION:										// Diverse Optionen einstellen
			...
			switch(U(wParam) & ~TVOP_WRITEOPTION) {
				case TVOP_AUTOEXPANDOFF:							// Icon Offset f�r TVS_EX_AUTOEXPANDICON
		...
		case TVM_GETITEM:											// Eintr�ge abfragen
		...
		case TVM_SETITEM:											// Eintr�ge einf�gen
		...
		case TVM_INSERTITEM:										// Eintr�ge einf�gen
		...
		case TVM_DELETEITEM:										// Eintr�ge l�schen
		...
		case TVM_FINDITEM:											// Eintr�ge suchen
		...
		case TVM_DELETECOLUMN:										// L�schen einer Salte im Header
		...
		case TVM_INSERTCOLUMN:										// Einf�gen einer Salte im Header
...
ColSet:
		...
		case TVM_SELECTCHILDS:										// Mehrere Eintr�ge ausw�hlen
		...
		case TVM_SELECTSUBITEM:										// Einen (Sub)Eintrag ausw�hlen
		...
		case TVM_SELECTDROP:										// Den unterstrichenen Eintrags ausw�hlen
		...
		case TVM_SELECTITEM:										// Einen Eintrag ausw�hlen
		...
		case TVM_GETCOUNT:											// Anzahl der Eintr�ge abfragen

			pData = GetHandle(hWnd);
			return pData->uTreeItemsCount;

		case TVM_GETINDENT:											// Einr�ckung abfragen

			pData = GetHandle(hWnd);
			return pData->iIndent;

		case TVM_SETINDENT:											// Einr�ckung einstellen
		...
		case TVM_GETITEMHEIGHT:										// Zeilenh�he abfragen

			pData = GetHandle(hWnd);
			return pData->iRowHeight;

		case TVM_SETITEMHEIGHT:										// Zeilenh�he abfragen
...
NextExp:
		...
		case TVM_SETINSERTMARK:										// Einf�gemarke eintellen
		...
		case TVM_SETITEMBKCOLOR:									// Hintergrundfarbe eines Eintrages �ndern
		...
		case TVM_SETITEMTEXTCOLOR:									// Textfarbe eines Eintrages �ndern
		...
		case TVM_SORTCHILDREN:										// Sortieren der Kindereintr�ge
		...
		case TVM_SETTOOLTIPS:										// Das Handle f�r das ToolTip-Fensters setzen
		...
		case TVM_SETUSERDATASIZE:									// Einstellen der Gr��e der User-Daten
		...
		case TVM_GETUSERDATASIZE:									// Abfragen der Gr��e der User-Daten
		...
		case TVM_SETCOLUMN:											// Einen Spalten-Header einstellen
			...
			if(pData->hHeader && U(wParam) < pData->uColumnCount) {
				...
				if(lRet && (pCol->mask & TVCF_FMT)) {			// Hat sich die Ausrichtung ver�ndert
					BYTE bAlign;
		...
		case TVM_COLUMNAUTOEDIT:									// AutoEdit f�r eine Spalte einstellen
		...
		case TVM_COLUMNAUTOICON:									// Icons f�r AutoEdit einstellen
...
//******************** Eintr�ge zeichnen **************************************
	for(; uPos < uMax; uPos++) {									// Alle Eintr�ge ausgeben
		uItem = pData->pItemPos[uPos];
		...
		if((pEntry->uState & TVIS_SELECTED) && (uStyleEx & TVS_EX_FULLROWMARK)) {
			...
			if(uPos & 1) {										// Farbe wechselweise �ndern
		...
		if(uStyle & TVS_HASLINES) {
			...
			for(i = iLevel; i > 0; i--) {							// Bereich vor Schaltfl�chen
		...
		if(uStyle & TVS_HASBUTTONS) {							// Fenster mit Schaltfl�chen ?
			...
			if(uStyle & TVS_HASLINES) {							// Linien unter Schaltfl�chen
				MoveToEx(hDc, iXpos, sArea.top | 1, NULL);
			...
			if(pEntry->bFlags & TVIX_HASBUTTON) {				// Schaltfl�chen zeichnen
				...
				if(pData->cGlyphOk) {							// Thema benutzen
					...
					// '+' statt '-' Schaltfl�sche zeichnenen
			...
			if(uStyle & TVS_HASLINES) {						// Nur Linien zeichnen ohne Schaltfl�chen
				sArea.right = sArea.left + iIndent;
				ExtTextOut(hDc, 0, 0, ETO_OPAQUE, &sArea, NULL, 0, NULL);
...
NoRootLines:
		...
		if(uBits & (TVIS_SELECTED | TVIS_DROPHILITED | TVIS_UNDERLINE | TVIS_TRACKED | TVIS_TEXTCOLOR | TVIS_FOCUSED)) {
			...
			if((uBits & TVIS_SELECTED) && pData->cHasFocus && uItem == pData->uSelectedItem && !pData->uSelectedSub) {
				...
				if(uBits & TVIS_DROPHILITED) {
					...
					if(uBits & TVIS_SELECTED) {				// Ist das Feld ohne Focus ausgew�hlt
						if(pData->cHasFocus) {
							uTempColor =     GetSysColor(COLOR_HIGHLIGHTTEXT);
							SelectObject(hDc, GetSysColorBrush(COLOR_HIGHLIGHT));
							SelectObject(hDc, GetStockObject(NULL_PEN));
			...
			// Ist der Text gr��er als die Spalte
		...
		if(uStyleEx & TVS_EX_ITEMLINES) {						// Linien um den Eintrag zeichnen
			...
			if(iLast > iStart) {								// Ist das Feld nach links einger�ckt gegen�ber dem oberen
				sArea.top--;
		...
		//************ Extraeintr�ge zeichnen *********************************

ExtraDraw:

		uNextColor		 = uOutColor;							// Hintergrundfarbe f�r die n�chste Spalte
		...
		for(uColumn = 1; uColumn <= pData->uColumnCount; uColumn++) {	// Extra Spalten zeichnen
			...
			if(!pExtra) {										// Leeres Feld zeichnen
				...
				if(pExtra->bCallback) {						// Text �ber Callback holen
				...
				if(uBits & (TVIS_SELECTED | TVIS_DROPHILITED | TVIS_UNDERLINE | TVIS_TRACKED | TVIS_TEXTCOLOR | TVIS_FOCUSED)) {
					...
					// Ist das Feld ohne Focus ausgew�hlt
	...
	if(sArea.top < sRect.bottom) {								// Untere Fl�che ohne Eintr�ge f�llen
